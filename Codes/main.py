...
#the 3 def below ensures that the ship is snapped to the drawn grid
...
def mainMenuScreen(window):
    ...
    for button in BUTTONS:
        if button.name in ['Easy Computer',]:
            button.active = True
            button.draw(window)
        else:
            button.active = False
            ...
            if event.button == 1:
                ...
                for button in BUTTONS:
                    if button.rect.collidepoint(pygame.mouse.get_pos()):
                        ...
                        elif (button.name == 'Easy Computer') and button.active == True:
